...
- The visitor implements the Visitor interface
...
from Structure.RNA_Molecule_test import RNA_Molecule
from utils import flattenMolecule

# --pdb format specific helpers
def _format_atom_info(serial, atom, residue, chain):
    """
...
def _format_molecule_info( rna_molecule):
    """
...
class PDBExportVisitor(Visitor):
    ...
    >responsible for exporting the RNA_Molecule object to a PDB file
    ...
    def visit_RNA_Molecule(self, rna_molecule:RNA_Molecule):
        '''
        method that allows the visitor to visit the RNA_Molecule object and export it to a PDB file
        '''

        atoms=flattenMolecule(rna_molecule)

        filename=rna_molecule.entry_id+"_PDBified.pdb"
        with open(filename, "w") as f:
            
            #Write molecule information
            molecule_info = _format_molecule_info(rna_molecule)
            ...
            for model_id, *atom_info in atoms:
                ...
                pdb_line = _format_atom_info(*atom_info)
                f.write(pdb_line)
